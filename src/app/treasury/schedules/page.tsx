...
export default function SchedulesPage() {
  ...
  const handleCreate = () => {
    ...
    try {
      ...
      createPaymentSchedule(deployment.id, {
        asset,
        recipient,
        amount: amountMinor,
        reason,
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <Panel edge>
          <div className="font-display text-[14px] font-semibold mb-4">New scheduled payment</div>
          ...
          <div className="mt-4">
            <Button variant="primary" size="sm" onClick={handleCreate} disabled={!recipient || !amount}>
              Create schedule
            </Button>
          </div>
        </Panel>
                ...
                <div key={s.id} className="p-4 space-y-3 border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="text-[13px] font-medium">{s.reason}</div>
                    ...
                    <div className="space-y-2 pl-1">
                      {occurrences.map((o) =>
                        o.status === "AWAITING_USER_INITIATION" ? (
                          ...
                          <div key={o.id} className="rounded-lg px-3 py-2 text-[11px] mono" style={{ background: "color-mix(in oklab, var(--bad) 8%, transparent)", color: "var(--bad)" }}>
